...
View: 
All
  http://xymedia.co.uk/phoenix/action/auto-login?user=<xsl:value-of select="/alert/user/login-name"/>&amp;hash=<xsl:value-of select="$hash-value"/>&amp;url=/action/tool?fwd=printarticle%26action=printarticle%26newsForm=true%26datefrom=<xsl:value-of select="alert-date/dd"/>-<xsl:value-of select="alert-date/mmm"/>-<xsl:value-of select="alert-date/yyyy"/>%26dateto=<xsl:value-of select="alert-date/dd"/>-<xsl:value-of select="alert-date/mmm"/>-<xsl:value-of select="alert-date/yyyy"/>
	 				<xsl:apply-templates select="sorting/sort-brief/sort-section" mode="index"/>
<xsl:value-of select="$n"/>
...
Prepared by xymedia - mail@mediagen.co.uk
	...
	<xsl:template match="sort-section" mode="index">
